    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
      ...
      >
        <div className="flex items-center justify-between mb-6 md:mb-8">
          <div>
            <h3 className="text-xl md:text-2xl font-semibold">{selectedUserToEdit ? 'Edit Member Permissions' : 'Invite Team Member'}</h3>
            <p className="text-xs md:text-sm text-slate-500">{selectedUserToEdit ? `Modify access for ${newUser.name}.` : 'Add a new collaborator to your agency or client workspace.'}</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="space-y-6 md:space-y-8">
          ...
          <div className="flex flex-col sm:flex-row gap-3 md:gap-4 pt-4">
            <button 
              onClick={onClose}
              className="w-full sm:flex-1 py-3 md:py-4 bg-white/5 hover:bg-white/10 rounded-xl md:rounded-2xl font-semibold transition-all text-sm md:text-base"
            >
              Cancel
            </button>
            <button 
              onClick={handleAddUser}
              className="w-full sm:flex-2 py-3 md:py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-xl md:rounded-2xl transition-all active:scale-[0.98] shadow-lg shadow-indigo-600/20 text-sm md:text-base"
            >
