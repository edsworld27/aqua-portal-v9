...
export const RoleBuilder: React.FC = () => {
  const { currentAgency, setAgencies, agencies, addLog, customPages, users, setUsers } = useAppContext();
      ...
      <div className="glass-card p-5 md:p-8 rounded-2xl md:rounded-3xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6 md:mb-8">
          ...
          <div className="flex gap-3">
            <button onClick={() => setEditingRole(null)} className="flex-1 md:flex-none px-4 py-2 bg-white/5 hover:bg-white/10 rounded-xl transition-colors text-xs md:text-sm">
              Cancel
            </button>
            <button onClick={handleSaveRole} className="flex-1 md:flex-none px-4 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-xl transition-colors flex items-center justify-center gap-2 text-xs md:text-sm">
        ...
        <div className="mb-6 md:mb-8">
          <label className="text-[10px] md:text-xs uppercase tracking-widest text-slate-500 mb-2 block">Role Name</label>
          <input 
            type="text" 
            value={editingRole.name}
            onChange={e => setEditingRole({...editingRole, name: e.target.value})}
            className="w-full max-w-md bg-slate-900/50 border border-white/10 rounded-xl p-2.5 md:p-3 text-xs md:text-sm text-white focus:outline-none focus:border-indigo-500"
          />
        </div>

        <div className="space-y-6 md:space-y-8">
          <div>
            <h3 className="text-base md:text-lg font-medium mb-4">Standard Permissions</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
              {standardViews.map(view => (
                <label key={view.id} className="flex items-center gap-3 p-3 md:p-4 bg-white/5 rounded-xl cursor-pointer hover:bg-white/10 transition-colors border border-white/5">
                  <input 
                    type="checkbox" 
          ...
          {customPages.length > 0 && (
            <div>
              <h3 className="text-base md:text-lg font-medium mb-4">Custom Page Permissions</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                {customPages.map(page => (
                  <label key={page.id} className="flex items-center gap-3 p-3 md:p-4 bg-white/5 rounded-xl cursor-pointer hover:bg-white/10 transition-colors border border-white/5">
                    <input 
                      type="checkbox" 
                      ...
                      className="w-4 h-4 rounded border-white/20 bg-slate-900 text-indigo-500 focus:ring-indigo-500 focus:ring-offset-slate-900"
                    />
                    <span className="text-xs md:text-sm font-medium">{page.title}</span>
                  </label>
                ))}
              </div>
            </div>
  ...
  return (
    <div className="glass-card p-5 md:p-8 rounded-2xl md:rounded-3xl">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6 md:mb-8">
        ...
        <button onClick={handleCreateRole} className="w-full md:w-auto px-4 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-xl transition-colors flex items-center justify-center gap-2 text-xs md:text-sm">
      ...
      <div className="space-y-3 md:space-y-4">
        {currentAgency.roles.length === 0 ? (
          ...
          currentAgency.roles.map(role => (
            <div key={role.id} className="flex items-center justify-between p-3 md:p-4 bg-white/5 rounded-xl md:rounded-2xl border border-white/5 hover:border-white/10 transition-colors">
              <div className="min-w-0">
                <div className="font-medium text-white flex items-center gap-2 text-sm md:text-base truncate">
                  {role.name}
                  {role.isMaster && <span className="px-2 py-0.5 bg-indigo-500/20 text-indigo-400 text-[8px] md:text-[10px] uppercase tracking-widest rounded-full font-bold shrink-0">System</span>}
                </div>
                <div className="text-[10px] md:text-xs text-slate-500 mt-1">{role.permissions.length} Permissions</div>
              </div>
